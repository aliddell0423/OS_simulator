...
/*
Funtion name: clearMetaDataList
...
Postcondiditon: displays all op codes in list
Exceptions: none
Note: none
*/
...
int getCommand( char *cmd, char *inputStr, int index )
{
   ...
   {
      // assign character from input to buffer string
                               ...
                               OpCodeType **opCodeDataHead, char *endStateMsg )
{
   ...
   // initialize op data pointer in case of return error
   *opCodeDataHead = NULL;
   ...
   // allocaste memory for the temporary data structure
      // function: malloc
   ...
   else
   {
      // set end state message
         // function: copyString
      copyString( endStateMsg, "Metadata corrupted op code" );
...
int getOpCommand( FILE *filePtr, OpCodeType *inData )
{
   ...
   // check for legitimate first string argument
   ...
   // check for app start if seconds argument
      // function: compareString
   ...
   // return complete op code command message
   return COMPLETE_OPCMD_FOUND_MSG;
}

/*
Funtion name: getStringArg
Algorithm: skips leading white space,
...
int getStringArg( char *strArg, char *inputStr, int index )
{
   ...
   // loop to skip white speace and comma
   while( inputStr[ index ] <= SPACE || inputStr[ index ] == COMMA )
   {
      // increment index
      index++;
...
/*
Funtion name: updateStartCount
Algorithm: updates number of "start" op commands found in a file
Precondition: count is >= 0, op string has "start" or other op name
Postcondiditon: if op string has "start", input count + 1 is returned;
...
/*
Funtion name: updateEndCount
Algorithm: updates number of "end" op commands found in a file
Precondition: count is >= 0, op string has "end" or other op name
Postcondiditon: if op string has "end", input count + 1 is returned;
                otherwise, input count is returned unchanged
Exceptions: none
Note: none
*/
...
/*
Funtion name: verifyFirstStringArg
...
{
   // check for all string arg 1 possibilities
      // function: compareString
...
/*
Funtion name: verifyValidCommand
